...
{
    public class MongoUserStore<TUser> : IUserPasswordStore<TUser>,
        ...
        where TUser : MongoIdentityUser
    {
        ...
        public MongoUserStore(IMongoCollection<TUser> users, IAutoincrement autoincrement)
        {
            _Users = users;
        ...
        {
            user.Id = _autoincrement.GetNext();
        ...
        public virtual Task<TUser> FindByIdAsync(string userId, CancellationToken token)
            => IsObjectId(userId)
                ? _Users.Find(u => u.Id.ToString() == userId).FirstOrDefaultAsync(token)
